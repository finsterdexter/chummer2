    ...
    public partial class SpiritControl : UserControl
    {
		...
		private void tsRemoveCharacter_Click(object sender, EventArgs e)
		{
			...
			if (MessageBox.Show(LanguageManager.Instance.GetString("Message_RemoveCharacterAssociation"), LanguageManager.Instance.GetString("MessageTitle_RemoveCharacterAssociation"), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
			{
                ...
                Uri uriApplication = new Uri(@GlobalOptions.ApplicationPath());
		...
		public void RebuildSpiritList(Guid guiTradition)
		{
			...
			foreach (XmlNode objXmlSpirit in objXmlDocument.SelectNodes("/chummer/traditions/tradition[id = \"" + guiTradition + "\"]/spirits/spirit"))
			{
		...
		private void CreateCritter(string strCritterName, int intForce)
		{
			...
			objCharacter.WalkRate = Convert.ToInt32(objXmlMetatype["walk"].InnerText);
            objCharacter.RunRate = Convert.ToInt32(objXmlDocument["run"].InnerText);
            objCharacter.SprintRate = Convert.ToInt32(objXmlDocument["sprint"].InnerText);
