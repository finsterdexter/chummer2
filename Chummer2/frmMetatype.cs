    ...
    public partial class frmMetatype : Form
    {
        ...
        void MetatypeSelected()
        {
            if (lstMetatypes.Text != "")
            {
				...
				_objCharacter.WalkRate = Convert.ToInt32(objXmlMetatype["walk"].InnerText);
                _objCharacter.RunRate = Convert.ToInt32(objXmlMetatype["run"].InnerText);
                _objCharacter.SprintRate = Convert.ToInt32(objXmlMetatype["sprint"].InnerText);
				...
				if (cboMetavariant.SelectedValue.ToString() != "None")
				{
					...
					if (objXmlMetavariant["bonus"] != null)
