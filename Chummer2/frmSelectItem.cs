...
namespace Chummer
{
	public partial class frmSelectItem : Form
	{
		...
		private void frmSelectItem_Load(object sender, EventArgs e)
		{
			...
			else if (_strMode == "Restricted")
			{
				...
				foreach (Weapon objWeapon in _objCharacter.Weapons)
				{
					...
					{
						if (objAccessory.TotalAvail.EndsWith(LanguageManager.Instance.GetString("String_AvailRestricted")) && !objAccessory.IncludedInParent)
						{
							ListItem objItem = new ListItem();
					...
					{
						if (objMod.TotalAvail.EndsWith(LanguageManager.Instance.GetString("String_AvailRestricted")) && !objMod.IncludedInParent)
						{
							ListItem objItem = new ListItem();
							objItem.Value = objMod.DisplayNameShort;
							objItem.Name = objMod.DisplayNameShort;
				...
				{
					if (objVehicle.TotalAvail.EndsWith(LanguageManager.Instance.GetString("String_AvailRestricted")))
					{
						ListItem objItem = new ListItem();
					...
					foreach (VehicleMod objMod in objVehicle.VehicleMods)
					{
						...
						foreach (Weapon objWeapon in objMod.Weapons)
						{
							...
							{
								if (objWeaponMod.TotalAvail.EndsWith(LanguageManager.Instance.GetString("String_AvailRestricted")) && !objWeaponMod.IncludedInParent)
								{
									ListItem objItem = new ListItem();
            ...
            else if (_strMode == "InherentLimits")
            {
                ListItem objMental = new ListItem();
                objMental.Value = "mentallimit";
                objMental.Name = LanguageManager.Instance.GetString("String_MentalLimit");
                lstItems.Add(objMental);

                ListItem objPhysical = new ListItem();
                objPhysical.Value = "physicallimit";
                objPhysical.Name = LanguageManager.Instance.GetString("String_PhysicalLimit");
                lstItems.Add(objPhysical);

                ListItem objSocial = new ListItem();
                objSocial.Value = "sociallimit";
                objSocial.Name = LanguageManager.Instance.GetString("String_SocialLimit");
                lstItems.Add(objSocial);
            }
        ...
        /// <summary>
        /// Manually set the Mode for the window to use.
        /// </summary>
