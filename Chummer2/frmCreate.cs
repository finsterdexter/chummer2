	...
	public partial class frmCreate : Form
	{
		...
		private void objCharacter_RESEnabledChanged(object sender)
		{
			...
			if (_objCharacter.RESEnabled)
			{
				...
				nudRES.Maximum = _objCharacter.RES.MetatypeMaximum;
		...
		private void mnuSpecialCyberzombie_Click(object sender, EventArgs e)
		{
			...
			_objImprovementManager.CreateImprovement("", Improvement.ImprovementSource.Cyberzombie, "Cyberzombie Qualities", Improvement.ImprovementType.FreeNegativeQualities, "", false, intResult * -1);
			...
			if (tabCharacterTabs.SelectedTab == tabStreetGear)
			{
				...
				if (tabStreetGearTabs.SelectedTab == tabWeapons)
				{
					...
					if (objXmlNode != null)
					{
						...
						if (blnAllowPaste)
						{
							objAccessory.Gears.Add(objGear);
							TreeNode objNode = new TreeNode();
							objNode.Text = objGear.DisplayName;
							objNode.Tag = objGear.InternalId;
			...
			if (tabCharacterTabs.SelectedTab == tabVehicles)
			{
				...
				if (objXmlNode != null)
				{
					...
					foreach (Vehicle objCharacterVehicle in _objCharacter.Vehicles)
					{
						...
						{
							objCharacterVehicle.Gears.Add(objGear);
							TreeNode objNode = new TreeNode();
							objNode.Text = objGear.DisplayName;
							objNode.Tag = objGear.InternalId;
					...
					Gear objVehicleGear = (Gear)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(Gear));

					if (objVehicleGear.Parent != null)
						objVehicle = (Vehicle)objVehicleGear.Parent;

					if (objVehicle == null)
						objVehicle = objTempVehicle;
					if (objVehicleGear != null)
					{
						objVehicleGear.Gears.Add(objGear);
		...
		private void nudBOD_ValueChanged(object sender, EventArgs e)
		{
			...
			// Make sure the number of points spent on Attributes does not exceed the number of points the character has chosen.
			if (!_objCharacter.IgnoreRules && !_blnFreestyle)
			{
                if (_objCharacter.BuildMethod == CharacterBuildMethod.Priority)
                {
                    int intOldValue = _objCharacter.BOD.Value;
                    _objCharacter.BOD.Value = Convert.ToInt32(nudBOD.Value);
                    if (_objCharacter.AttributePointsSpent() > _objCharacter.AttributePoints)
                    {
                        _blnLoading = true;
                        nudBOD.Value = intOldValue;
                        _objCharacter.BOD.Value = intOldValue;
                        ShowAttributeBPRule();
                        _blnLoading = false;
                    }
                }
		...
		private void nudAGI_ValueChanged(object sender, EventArgs e)
		{
            ...
            if (!_objCharacter.IgnoreRules && !_blnFreestyle)
            {
                if (_objCharacter.BuildMethod == CharacterBuildMethod.Priority)
                {
                    int intOldValue = _objCharacter.AGI.Value;
                    _objCharacter.AGI.Value = Convert.ToInt32(nudAGI.Value);
                    if (_objCharacter.AttributePointsSpent() > _objCharacter.AttributePoints)
                    {
                        _blnLoading = true;
                        nudAGI.Value = intOldValue;
                        _objCharacter.AGI.Value = intOldValue;
                        ShowAttributeBPRule();
                        _blnLoading = false;
                    }
                }
		...
		private void nudREA_ValueChanged(object sender, EventArgs e)
		{
            ...
            if (!_objCharacter.IgnoreRules && !_blnFreestyle)
            {
                if (_objCharacter.BuildMethod == CharacterBuildMethod.Priority)
                {
                    int intOldValue = _objCharacter.REA.Value;
                    _objCharacter.REA.Value = Convert.ToInt32(nudREA.Value);
                    if (_objCharacter.AttributePointsSpent() > _objCharacter.AttributePoints)
                    {
                        _blnLoading = true;
                        nudREA.Value = intOldValue;
                        _objCharacter.REA.Value = intOldValue;
                        ShowAttributeBPRule();
                        _blnLoading = false;
                    }
                }
		...
		private void nudSTR_ValueChanged(object sender, EventArgs e)
		{
            ...
            if (!_objCharacter.IgnoreRules && !_blnFreestyle)
            {
                if (_objCharacter.BuildMethod == CharacterBuildMethod.Priority)
                {
                    int intOldValue = _objCharacter.STR.Value;
                    _objCharacter.STR.Value = Convert.ToInt32(nudSTR.Value);
                    if (_objCharacter.AttributePointsSpent() > _objCharacter.AttributePoints)
                    {
                        _blnLoading = true;
                        nudSTR.Value = intOldValue;
                        _objCharacter.STR.Value = intOldValue;
                        ShowAttributeBPRule();
                        _blnLoading = false;
                    }
                }
		...
		private void nudCHA_ValueChanged(object sender, EventArgs e)
		{
            ...
            if (!_objCharacter.IgnoreRules && !_blnFreestyle)
            {
                if (_objCharacter.BuildMethod == CharacterBuildMethod.Priority)
                {
                    if (_objCharacter.AttributePointsSpent() + 1 > _objCharacter.AttributePoints)
                    {
                        try
                        {
                            _blnLoading = true;
		...
		private void nudINT_ValueChanged(object sender, EventArgs e)
		{
            ...
            if (!_objCharacter.IgnoreRules && !_blnFreestyle)
            {
                if (_objCharacter.BuildMethod == CharacterBuildMethod.Priority)
                {
                    int intOldValue = _objCharacter.INT.Value;
                    _objCharacter.INT.Value = Convert.ToInt32(nudINT.Value);
                    if (_objCharacter.AttributePointsSpent() > _objCharacter.AttributePoints)
                    {
                        _blnLoading = true;
                        nudINT.Value = intOldValue;
                        _objCharacter.INT.Value = intOldValue;
                        ShowAttributeBPRule();
                        _blnLoading = false;
                    }
                }
		...
		private void nudWIL_ValueChanged(object sender, EventArgs e)
		{
            ...
            if (!_objCharacter.IgnoreRules && !_blnFreestyle)
            {
                if (_objCharacter.BuildMethod == CharacterBuildMethod.Priority)
                {
                    int intOldValue = _objCharacter.WIL.Value;
                    _objCharacter.WIL.Value = Convert.ToInt32(nudWIL.Value);
                    if (_objCharacter.AttributePointsSpent() > _objCharacter.AttributePoints)
                    {
                        _blnLoading = true;
                        nudWIL.Value = intOldValue;
                        _objCharacter.WIL.Value = intOldValue;
                        ShowAttributeBPRule();
                        _blnLoading = false;
                    }
                }
		...
		private void nudEDG_ValueChanged(object sender, EventArgs e)
		{
            ...
            // Make sure the number of points spent on Special Attributes does not exceed the number of points the character has chosen.
            if (!_objCharacter.IgnoreRules && !_blnFreestyle)
            {
                if (_objCharacter.BuildMethod == CharacterBuildMethod.Priority)
                {
                    int intOldValue = _objCharacter.EDG.Value;
                    _objCharacter.EDG.Value = Convert.ToInt32(nudEDG.Value);
                    if (_objCharacter.SpecialAttributePointsSpent() > _objCharacter.SpecialAttributePoints)
                    {
                        _blnLoading = true;
                        nudEDG.Value = intOldValue;
                        _objCharacter.EDG.Value = intOldValue;
                        ShowAttributeBPRule(true);
                        _blnLoading = false;
                    }
                }
            }
		...
		private void nudMAG_ValueChanged(object sender, EventArgs e)
		{
            ...
            if (!_objCharacter.IgnoreRules && !_blnFreestyle)
            {
                if (_objCharacter.BuildMethod == CharacterBuildMethod.Priority)
                {
                    int intOldValue = _objCharacter.MAG.Value;
                    _objCharacter.MAG.Value = Convert.ToInt32(nudMAG.Value);
                    if (_objCharacter.SpecialAttributePointsSpent() > _objCharacter.SpecialAttributePoints)
                    {
                        _blnLoading = true;
                        nudMAG.Value = intOldValue;
                        _objCharacter.MAG.Value = intOldValue;
                        ShowAttributeBPRule(true);
                        _blnLoading = false;
                    }
                }
		...
		private void nudRES_ValueChanged(object sender, EventArgs e)
		{
            ...
            if (!_objCharacter.IgnoreRules && !_blnFreestyle)
            {
                if (_objCharacter.BuildMethod == CharacterBuildMethod.Priority)
                {
                    int intOldValue = _objCharacter.RES.Value;
                    _objCharacter.RES.Value = Convert.ToInt32(nudRES.Value);
                    if (_objCharacter.SpecialAttributePointsSpent() > _objCharacter.SpecialAttributePoints)
                    {
                        _blnLoading = true;
                        nudRES.Value = intOldValue;
                        _objCharacter.RES.Value = intOldValue;
                        ShowAttributeBPRule(true);
                        _blnLoading = false;
                    }
                }
		...
		private void objEnemy_ConnectionRatingChanged(Object sender)
		{
			...
			foreach (Quality objQuality in _objCharacter.Qualities)
			{
				if (objQuality.Type == QualityType.Negative && objQuality.ContributeToLimit)
					intNegativeQualityBP += objQuality.Cost;
			}
			...
			if (objQuality.Type == QualityType.Negative)
			{
				...
				foreach (Quality objCharacterQuality in _objCharacter.Qualities)
				{
					if (objCharacterQuality.Type == QualityType.Negative && objCharacterQuality.ContributeToLimit)
						intBP += objCharacterQuality.Cost;
				}
				if (_objCharacter.BuildMethod == CharacterBuildMethod.Karma)
					intBP *= _objOptions.KarmaQuality;
		...
		private void treQualities_AfterSelect(object sender, TreeViewEventArgs e)
		{
			...
			if (_objCharacter.BuildMethod == CharacterBuildMethod.BP)
				lblQualityBP.Text = objQuality.Cost.ToString() + " " + LanguageManager.Instance.GetString("String_BP");
			else
				...
				else if (treArmor.SelectedNode.Level > 1)
				{
					...
					if (objMod != null)
					{
						...
						if (chkArmorEquipped.Checked)
						{
							...
							if (objMod.Bonus != null && ((ArmorMod)objMod.Parent).Equipped)
								_objImprovementManager.CreateImprovements(Improvement.ImprovementSource.ArmorMod, objMod.InternalId, objMod.Bonus, false, objMod.Rating, objMod.DisplayNameShort);
						}
						else
						{
		...
		private void nudVehicleRating_ValueChanged(object sender, EventArgs e)
		{
			...
			else if (treVehicles.SelectedNode.Level > 2)
			{
				...
				else
				{
					...
					bool blnCyberware = false;
					Cyberware objCyberware = (Cyberware)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(Cyberware));
					if (objCyberware != null)
						blnCyberware = true;
		...
		private void chkVehicleHomeNode_CheckedChanged(object sender, EventArgs e)
		{
			...
			{
				Commlink objGear = new Commlink(_objCharacter);
				objGear = (Commlink)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(Gear));
		...
		private void chkVehicleBlackMarketDiscount_CheckedChanged(object sender, EventArgs e)
		{
			...
			try
			{
				...
				WeaponAccessory objWeaponAccessory = (WeaponAccessory)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(WeaponAccessory));
				if (objWeaponAccessory != null)
					objWeaponAccessory.DiscountCost = chkVehicleBlackMarketDiscount.Checked;

				WeaponMod objWeaponMod = (WeaponMod)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(WeaponMod));
		...
		/// </summary>
		public void ShowAttributeBPRule(bool blnSpecialAttribute = false)
		{
            if (_objCharacter.BuildMethod == CharacterBuildMethod.BP || _objCharacter.BuildMethod == CharacterBuildMethod.Karma)
            {
                int intPoints = 0;
                string strMethod = "";
                if (_objCharacter.BuildMethod == CharacterBuildMethod.BP)
            ...
            else if (_objCharacter.BuildMethod == CharacterBuildMethod.Priority)
            {
                int intPoints = 0;
                string strMessage = "";
                if (!blnSpecialAttribute)
                {
                    intPoints = _objCharacter.AttributePoints;
                    strMessage = LanguageManager.Instance.GetString("Message_AttributeBuildPoints");
                }
                else
                {
                    intPoints = _objCharacter.SpecialAttributePoints;
                    strMessage = LanguageManager.Instance.GetString("Message_SpecialAttributeBuildPoints");
                }

                MessageBox.Show(string.Format(strMessage, intPoints.ToString() + " " + LanguageManager.Instance.GetString("String_Points")), LanguageManager.Instance.GetString("MessageTitle_Attribute"), MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
		}
		...
		private bool CanImproveAttribute(string strAttribute)
		{
			...
			foreach (Control objControl in panAttributes.Controls)
			{
				...
				{
					// Edge, Magic, and Resonance are not part of the rule where only one Attribute can be at is maximum (only applies to Physical and Mental Attributes).
		...
		private int CalculateBP()
		{
			...
			foreach (Quality objQuality in _objCharacter.Qualities)
			{
				if (objQuality.Type == QualityType.Positive && objQuality.ContributeToBP)
				{
					if (_objCharacter.BuildMethod == CharacterBuildMethod.BP)
					{
						intPointsRemain -= objQuality.Cost;
						intPointsUsed += objQuality.Cost;
					}
					else
					{
						intPointsRemain -= (objQuality.Cost * _objOptions.KarmaQuality);
						intPointsUsed += (objQuality.Cost * _objOptions.KarmaQuality);
					}
				}
			}
			...
			foreach (Quality objQuality in _objCharacter.Qualities)
			{
				if (objQuality.Type == QualityType.Negative && objQuality.ContributeToBP)
				{
					if (_objCharacter.BuildMethod == CharacterBuildMethod.BP)
					{
						intPointsRemain -= objQuality.Cost;
						intPointsUsed += objQuality.Cost;
						intNegativePoints += objQuality.Cost;
					}
					else
					{
						intPointsRemain -= (objQuality.Cost * _objOptions.KarmaQuality);
						intPointsUsed += (objQuality.Cost * _objOptions.KarmaQuality);
						intNegativePoints += (objQuality.Cost * _objOptions.KarmaQuality);
					}
				}
			}
			...
			intFreestyleBP += intPointsUsed;
			lblAttributesBP.Text = String.Format("{0} " + strPoints, _objCharacter.AttributePointsSpent());
			intPointsRemain -= CalculateSpecialAttributeBP();
			intPointsUsed = CalculateSpecialAttributeBP();
			lblSpecialAttributesBP.Text = String.Format("{0} " + strPoints, _objCharacter.SpecialAttributePointsSpent());
			intFreestyleBP += intPointsUsed;
			...
			if (!_blnSkipUpdate)
			{
				...
				if (_objCharacter.MetatypeCategory == "Mutant Critters")
				{
					...
					if (intEssencePenalty < 0)
                        _objImprovementManager.CreateImprovement("", Improvement.ImprovementSource.MutantCritter, "", Improvement.ImprovementType.Essence, "", false, intEssencePenalty);
					if (intIntuitionPenalty < 0)
                        _objImprovementManager.CreateImprovement("INT", Improvement.ImprovementSource.MutantCritter, "", Improvement.ImprovementType.Attribute, "", false, 0, 1, 0, 0, intIntuitionPenalty);
		...
		public void RefreshSelectedArmor()
		{
			...
			else if (objFoundEquipment.GetType() == typeof(ArmorMod))
			{
				...
				if (objSelectedMod.MaxRating > 1)
				{
					_blnSkipRefresh = true;
					nudArmorRating.Maximum = objSelectedMod.MaxRating;
		...
		private void RefreshSelectedVehicle()
		{
			...
			else if (objFoundEquipment.GetType() == typeof(VehicleMod))
			{
				...
				Vehicle objSelectedVehicle = (Vehicle)objMod.Parent;
				...
				if (Convert.ToInt32(objMod.MaxRating) > 0)
				{
					...
					nudVehicleRating.Enabled = !objMod.IncludedInParent;
		...
		public void AddPACKSKit()
		{
			...
			{	
				...
				{
					XmlNode objXmlPowerNode = objXmlPowerDocument.SelectSingleNode("/chummer/powers/power[id = \"" + objXmlPower["id"].InnerText + "\"]");
			...
			if (objXmlKit["armors"] != null)
			{
				...
				{
					XmlNode objXmlArmorNode = objXmlArmorDocument.SelectSingleNode("/chummer/armors/armor[id = \"" + objXmlArmor["id"].InnerText + "\"]");
					...
					if (objXmlArmor["mods"] != null)
					{
						foreach (XmlNode objXmlMod in objXmlArmor.SelectNodes("mods/mod"))
						{
							List<Weapon> lstWeapons = new List<Weapon>();
							List<TreeNode> lstWeaponNodes = new List<TreeNode>();
							XmlNode objXmlModNode = objXmlArmorDocument.SelectSingleNode("/chummer/mods/mod[id = \"" + objXmlMod["id"].InnerText + "\"]");
							ArmorMod objMod = new ArmorMod(_objCharacter);
			...
			if (objXmlKit["weapons"] != null)
			{
				...
				foreach (XmlNode objXmlWeapon in objXmlKit.SelectNodes("weapons/weapon"))
				{
					...
					{
						XmlNode objXmlUnderbarrelNode = objXmlWeaponDocument.SelectSingleNode("/chummer/weapons/weapon[id = \"" + objXmlWeapon["underbarrel"].InnerText + "\"]");
						...
						objWeapon.Weapons.Add(objUnderbarrelWeapon);
			...
			if (objXmlKit["cyberwares"] != null)
			{
				...
				foreach (XmlNode objXmlCyberware in objXmlKit.SelectNodes("cyberwares/cyberware"))
				{
					...
					XmlNode objXmlCyberwareNode = objXmlCyberwareDocument.SelectSingleNode("/chummer/cyberwares/cyberware[id = \"" + objXmlCyberware["id"].InnerText + "\"]");
					...
					if (objXmlCyberware["cyberwares"] != null)
					{
						foreach (XmlNode objXmlChild in objXmlCyberware.SelectNodes("cyberwares/cyberware"))
						{
							...
							objCyberware.Cyberwares.Add(objChildCyberware);
			...
			if (objXmlKit["biowares"] != null)
			{
				...
				foreach (XmlNode objXmlBioware in objXmlKit.SelectNodes("biowares/bioware"))
				{
					...
					XmlNode objXmlBiowareNode = objXmlBiowareDocument.SelectSingleNode("/chummer/biowares/bioware[id = \"" + objXmlBioware["id"].InnerText + "\"]");
			...
			if (objXmlKit["vehicles"] != null)
			{
				...
				foreach (XmlNode objXmlVehicle in objXmlKit.SelectNodes("vehicles/vehicle"))
				{
					...
					foreach (Gear objSensorGear in objVehicle.Gears)
					{
					...
					if (objXmlVehicle["mods"] != null)
					{
						foreach (XmlNode objXmlMod in objXmlVehicle.SelectNodes("mods/mod"))
						{
							...
							XmlNode objXmlModNode = objXmlVehicleDocument.SelectSingleNode("/chummer/mods/mod[id = \"" + objXmlMod["id"].InnerText + "\"]");
							objMod.Create(objXmlModNode, objModNode, intRating);
							objVehicle.VehicleMods.Add(objMod);
					...
					if (objXmlVehicle["gears"] != null)
					{
						...
						foreach (XmlNode objXmlGear in objXmlVehicle.SelectNodes("gears/gear"))
						{
							...
							XmlNode objXmlGearNode = objXmlGearDocument.SelectSingleNode("/chummer/gears/gear[id = \"" + objXmlGear["id"].InnerText + "\"]");
							...
							{
								objVehicle.Gears.Remove(objDefaultSensor);
		...
		private void MoveControls()
		{
            ...
            lblVehicleSeatsLabel.Left = lblVehicleCost.Left + 53;
            lblVehicleSeats.Left = lblVehicleSeatsLabel.Left + lblVehicleSeatsLabel.Width + 6;
            lblVehicleSlotsLabel.Left = lblVehicleSeats.Left + 53;
