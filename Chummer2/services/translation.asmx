...
public class translation : System.Web.Services.WebService
{
	...
	public bool CanUploadLanguage(string strUserName)
	{
		...
		{
			objCommand.CommandText = "SELECT * FROM tblOmaeUserLanguagePermissionSR5 WHERE strUserName = '" + SQLSafe(strUserName) + "'";
	...
	public int UploadLanguage(string strUserName, string strFileName, byte[] bytFile)
	{
		...
		if (strUserName != "Nebular")
		{
			objCommand.CommandText = "SELECT * FROM tblOmaeUserLanguagePermissionSR5 WHERE strUserName = '" + SQLSafe(strUserName) + "' AND strFileName = '" + SQLSafe(strFileName) + "'";
			objReader = objCommand.ExecuteReader();
			if (!objReader.Read())
			{
				objReader.Close();
		...
		objCommand.CommandText = "SELECT * FROM tblLanguageSR5 WHERE strFile = '" + SQLSafe(strFileName) + "'";
		objReader = objCommand.ExecuteReader();
		if (objReader.Read())
		{
		...
		string strSavePath = Path.Combine(Path.Combine(Server.MapPath("/dev/chummersr5"), "lang"), FileSafe(strFileName));
		...
		string strManifestPath = Path.Combine(Server.MapPath("/dev/chummersr5"), "manifestlang.xml");
