		...
		private void cboCategory_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			else
			{
				...
				if (objXmlCritter["optionalpowers"] != null)
				{
					...
					{
						XmlNode objXmlPower = _objXmlDocument.SelectSingleNode("/chummer/powers/power[id = \"" + objXmlCritterPower.InnerText + "\"]");
						TreeNode objNode = new TreeNode();
						objNode.Tag = objXmlPower["id"].InnerText;
						if (objXmlPower["translate"] != null)
							objNode.Text = objXmlPower["translate"].InnerText;
							...
							if (!blnPhysicalPresence){
								if (objXmlPower["name"].InnerText == "Materialization")
								{
									...
									if (!blnFound)
									{
										...
										objPossessionNode.Tag = objXmlPossessionPower["id"].InnerText;
										if (objXmlPower["translate"] != null)
