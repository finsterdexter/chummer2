...
namespace Chummer
{
	public partial class frmSelectMentorSpirit : Form
	{
		...
		private void frmSelectMentorSpirit_Load(object sender, EventArgs e)
		{
            ...
            ListItem objMagician = new ListItem();
            objMagician.Value = "Magician";
            objMagician.Name = LanguageManager.Instance.GetString("Label_Magician");
            
            ListItem objAdept = new ListItem();
            objAdept.Value = "Adept";
            objMagician.Name = LanguageManager.Instance.GetString("Label_Adept");

            List<ListItem> lstAspect = new List<ListItem>();
            if (_objCharacter.MagicianEnabled)
                lstAspect.Add(objMagician);
            if (_objCharacter.AdeptEnabled)
                lstAspect.Add(objAdept);

            cboAspect.ValueMember = "Value";
            cboAspect.DisplayMember = "Name";
            cboAspect.DataSource = lstAspect;
        ...
        /// <summary>
        /// Second Bonus Node for the Mentor and Aspect that was selected.
        /// </summary>
        public XmlNode BonusNode2
        {
            get
            {
                return _nodBonus2;
		...
		private void AcceptForm()
		{
			if (lstMentor.Text != "")
			{
				...
				XmlNode objXmlMentor = _objXmlDocument.SelectSingleNode("/chummer/mentors/mentor[name = \"" + lstMentor.SelectedValue + "\"]");
				if (objXmlMentor["bonus"] != null)
					_nodBonus = objXmlMentor.SelectSingleNode("bonus");

                if (cboAspect.SelectedValue.ToString() == "Magician")
                {
                    if (objXmlMentor["bonusm"] != null)
                        _nodBonus2 = objXmlMentor.SelectSingleNode("bonusm");
                }
                else
                {
                    if (objXmlMentor["bonusa"] != null)
                        _nodBonus2 = objXmlMentor.SelectSingleNode("bonusa");
                }
