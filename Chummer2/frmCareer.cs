...
namespace Chummer
{
	public partial class frmCareer : Form
	{
		...
		private void frmCareer_Load(object sender, EventArgs e)
		{
			...
			{
				objXmlDocument = XmlManager.Instance.Load("traditions.xml");
				XmlNode objXmlTradition = objXmlDocument.SelectSingleNode("/chummer/traditions/tradition[id = \"" + _objCharacter.MagicTradition + "\"]");
			...
			{
				objXmlDocument = XmlManager.Instance.Load("streams.xml");
				XmlNode objXmlTradition = objXmlDocument.SelectSingleNode("/chummer/traditions/tradition[id = \"" + _objCharacter.TechnomancerStream + "\"]");
        ...
        private void objCharacter_SensitiveSystemChanged(object sender)
        {
            if (_blnReapplyImprovements)
                return;

            // Change to the status of Sensitive System being enabled.
            if (_objCharacter.SensitiveSystem)
            {
                // If Sensitive System is being added, disable the Add Bioware button.
                cmdAddBioware.Enabled = false;
            }
            else
            {
                // If Sensitive System is being removed, re-enable the Add Bioware button.
                cmdAddBioware.Enabled = true;
            }
        }
		...
		private void mnuSpecialReduceAttribute_Click(object sender, EventArgs e)
		{
			...
			if (!frmPickAttribute.DoNotAffectMetatypeMaximum)
                _objImprovementManager.CreateImprovement(frmPickAttribute.SelectedAttribute, Improvement.ImprovementSource.AttributeLoss, "Attribute Loss", Improvement.ImprovementType.Attribute, "", false, 0, 1, 0, -1);
		...
		private void mnuSpecialReapplyImprovements_Click(object sender, EventArgs e)
		{
			...
			foreach (Power objPower in _objCharacter.Powers)
			{
				...
				XmlNode objNode = objXmlDocument.SelectSingleNode("/chummer/powers/power[id = \"" + objPower.ExternalId + "\"]");
				if (objNode != null)
				{
			...
			foreach (Cyberware objCyberware in _objCharacter.Cyberware)
			{
				if (objCyberware.SourceType == Improvement.ImprovementSource.Cyberware)
				{
					...
					XmlNode objNode = objXmlDocument.SelectSingleNode("/chummer/cyberwares/cyberware[id = \"" + objCyberware.ExternalId + "\"]");
				...
				else
				{
					...
					XmlNode objNode = objXmlDocument.SelectSingleNode("/chummer/biowares/bioware[id = \"" + objCyberware.ExternalId + "\"]");
			...
			foreach (Armor objArmor in _objCharacter.Armor)
			{
				...
				XmlNode objNode = objXmlDocument.SelectSingleNode("/chummer/armors/armor[id = \"" + objArmor.ExternalId + "\"]");
				...
				{
					XmlNode objChild = objXmlDocument.SelectSingleNode("/chummer/mods/mod[id = \"" + objMod.ExternalId + "\"]");
				...
				foreach (Gear objGear in objArmor.Gears)
				{
					...
					XmlNode objChild = objXmlDocument.SelectSingleNode("/chummer/gears/gear[id = \"" + objGear.ExternalId + "\"]");
			...
			foreach (Gear objGear in _objCharacter.Gear)
			{
				...
				XmlNode objNode = objXmlDocument.SelectSingleNode("/chummer/gears/gear[id = \"" + objGear.ExternalId + "\"]");
				...
				foreach (Gear objPlugin in objGear.Gears)
				{
					string strPluginSelected = objPlugin.Extra;
					XmlNode objChild = objXmlDocument.SelectSingleNode("/chummer/gears/gear[id = \"" + objPlugin.ExternalId + "\"]");
					...
					foreach (Gear objSubPlugin in objPlugin.Gears)
					{
						string strSubPluginSelected = objSubPlugin.Extra;
						XmlNode objSubChild = objXmlDocument.SelectSingleNode("/chummer/gears/gear[id = \"" + objSubPlugin.ExternalId + "\"]");
		...
		private void mnuSpecialPossessInanimate_Click(object sender, EventArgs e)
		{
			...
			objMerge.WalkRate = Convert.ToInt32(objSelected["walk"].InnerText);
            objMerge.RunRate = Convert.ToInt32(objSelected["run"].InnerText);
            objMerge.SprintRate = Convert.ToInt32(objSelected["sprint"].InnerText);
			...
			if (objSelected["powers"] != null)
			{
				...
				{
					XmlNode objXmlCritterPower = objXmlPowerDoc.SelectSingleNode("/chummer/powers/power[id = \"" + objXmlPower.InnerText + "\"]");
			...
			if (tabCharacterTabs.SelectedTab == tabStreetGear)
			{
				...
				if (tabStreetGearTabs.SelectedTab == tabArmor)
				{
					try
					{
						...
						Armor objCopyArmor = (Armor)_objFunctions.FindEquipment(treArmor.SelectedNode.Tag.ToString(), _objCharacter.Armor, typeof(Armor));
						...
						Gear objCopyGear = (Gear)_objFunctions.FindEquipment(treArmor.SelectedNode.Tag.ToString(), _objCharacter.Armor, typeof(Gear));
						...
						if (objCopyGear != null)
						{
							...
							if (objCopyGear.WeaponID != Guid.Empty.ToString())
							{
								...
								Weapon objCopyWeapon = (Weapon)_objFunctions.FindEquipment(objCopyGear.WeaponID, _objCharacter.Weapons, typeof(Weapon));
				...
				if (tabStreetGearTabs.SelectedTab == tabWeapons)
				{
					try
					{
						...
						Weapon objCopyWeapon = (Weapon)_objFunctions.FindEquipment(treWeapons.SelectedNode.Tag.ToString(), _objCharacter.Weapons, typeof(Weapon));
						...
						Gear objCopyGear = (Gear)_objFunctions.FindEquipment(treWeapons.SelectedNode.Tag.ToString(), _objCharacter.Weapons, typeof(Gear));
						...
						if (objCopyGear != null)
						{
							...
							{
								// Copy any Weapon that comes with the Gear.
								Weapon objCopyGearWeapon = (Weapon)_objFunctions.FindEquipment(objCopyGear.WeaponID, _objCharacter.Weapons, typeof(Weapon));
				...
				if (tabStreetGearTabs.SelectedTab == tabGear)
				{
					try
					{
						...
						Gear objCopyGear = (Gear)_objFunctions.FindEquipment(treGear.SelectedNode.Tag.ToString(), _objCharacter.Gear, typeof(Gear));
			...
			if (tabCharacterTabs.SelectedTab == tabVehicles)
			{
				try
				{
					if (treVehicles.SelectedNode.Level == 1)
					{
						...
						Vehicle objCopyVehicle = (Vehicle)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(Vehicle));
					...
					{
						Gear objCopyGear = (Gear)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(Gear));
						...
						{
							foreach (VehicleMod objMod in objCharacterVehicle.VehicleMods)
							{
								Weapon objCopyWeapon = (Weapon)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), objMod.Weapons, typeof(Weapon));
								if (objCopyWeapon != null)
								{
									// Do not let the user copy Gear or Cyberware Weapons.
									if (objCopyWeapon.Category == "Gear" || objCopyWeapon.Category.StartsWith("Cyberware"))
		...
		private void cmdDeleteCyberware_Click(object sender, EventArgs e)
		{
			try
			{
				if (treCyberware.SelectedNode.Level > 0)
				{
					...
					objCyberware = (Cyberware)_objFunctions.FindEquipment(treCyberware.SelectedNode.Tag.ToString(), _objCharacter.Cyberware, typeof(Cyberware));
					if (objCyberware != null)
					{
						blnFound = true;
						objParent = (Cyberware)objCyberware.Parent;
					}
					...
					else
					{
						...
						Gear objGear = (Gear)_objFunctions.FindEquipment(treCyberware.SelectedNode.Tag.ToString(), _objCharacter.Cyberware, typeof(Gear));

						if (objGear.Parent != null)
							objCyberware = (Cyberware)objGear.Parent;

						if (objGear.Parent == null)
							objCyberware.Gears.Remove(objGear);
						else
							((Gear)objGear.Parent).Gears.Remove(objGear);
						_objFunctions.DeleteGear(objGear, treWeapons, _objImprovementManager);
					}
					...
					if (objParent != null)
						objParent.Cyberwares.Remove(objCyberware);
				}
		...
		private void cmdDeleteArmor_Click(object sender, EventArgs e)
		{
			...
			try
			{
				if (treArmor.SelectedNode.Level == 0)
				{
					...
					{
						Armor objArmor = (Armor)_objFunctions.FindEquipment(objNode.Tag.ToString(), _objCharacter.Armor, typeof(Armor));
						...
						foreach (Gear objChild in objArmor.Gears)
						{
							TreeNode nodChildNode = new TreeNode();
							nodChildNode.Text = objChild.DisplayName;
							nodChildNode.Tag = objChild.InternalId;
				...
				{
					bool blnIsMod = false;
					ArmorMod objMod = (ArmorMod)_objFunctions.FindEquipment(treArmor.SelectedNode.Tag.ToString(), _objCharacter.Armor, typeof(ArmorMod));
					if (objMod != null)
						blnIsMod = true;
					...
					else
					{
						...
						if (objGear.Parent != null)
							objSelectedArmor = (Armor)objGear.Parent;

						_objFunctions.DeleteGear(objGear, treWeapons, _objImprovementManager);
						objSelectedArmor.Gears.Remove(objGear);
					}
					treArmor.SelectedNode.Remove();
				}
				else if (treArmor.SelectedNode.Level > 2)
		...
		private void cmdDeleteWeapon_Click(object sender, EventArgs e)
		{
			...
			try
			{
				if (treWeapons.SelectedNode.Level == 0)
				{
					...
					{
						Weapon objWeapon = new Weapon(_objCharacter);
						objWeapon = (Weapon)_objFunctions.FindEquipment(objNode.Tag.ToString(), _objCharacter.Weapons, typeof(Weapon));
				...
				if (treWeapons.SelectedNode.Level > 0)
				{
					...
					if (treWeapons.SelectedNode.Level == 1)
					{
						...
						{
							foreach (Gear objGear in objDelAccessory.Gears)
								_objFunctions.DeleteGear(objGear, treWeapons, _objImprovementManager);
						}
					...
					else
					{
						...
						{
							if (objCharacterWeapon.Weapons.Count > 0)
							{
								foreach (Weapon objUnderbarrelWeapon in objCharacterWeapon.Weapons)
								{
									if (objUnderbarrelWeapon.InternalId == treWeapons.SelectedNode.Tag.ToString())
									{
										objCharacterWeapon.Weapons.Remove(objUnderbarrelWeapon);
										treWeapons.SelectedNode.Remove();
										return;
									}
								}
						...
						if (!blnAccessory)
						{
							...
							bool blnMod = false;
							WeaponMod objMod = (WeaponMod)_objFunctions.FindEquipment(treWeapons.SelectedNode.Tag.ToString(), _objCharacter.Weapons, typeof(WeaponMod));
							if (objMod != null)
								blnMod = true;
							...
							else
							{
								...
								Gear objGear = (Gear)_objFunctions.FindEquipment(treWeapons.SelectedNode.Tag.ToString(), _objCharacter.Weapons, typeof(Gear));
								_objFunctions.DeleteGear(objGear, treWeapons, _objImprovementManager);

								if (objGear.Parent != null)
									objAccessory = (WeaponAccessory)objGear.Parent;

								if (objGear.Parent == null)
									objAccessory.Gears.Remove(objGear);
								else
									((Gear)objGear.Parent).Gears.Remove(objGear);
								treWeapons.SelectedNode.Remove();
							}
			...
			try
			{
				if (treGear.SelectedNode.Level == 0)
				{
					...
					foreach (TreeNode objNode in treGear.SelectedNode.Nodes)
					{
						...
						objGear = (Gear)_objFunctions.FindEquipment(objNode.Tag.ToString(), _objCharacter.Gear, typeof(Gear));
				...
				if (treGear.SelectedNode.Level > 0)
				{
					...
					Gear objGear = new Gear(_objCharacter);
					objGear = (Gear)_objFunctions.FindEquipment(treGear.SelectedNode.Tag.ToString(), _objCharacter.Gear, typeof(Gear));
					Gear objParent = new Gear(_objCharacter);
					objParent = (Gear)_objFunctions.FindEquipment(treGear.SelectedNode.Parent.Tag.ToString(), _objCharacter.Gear, typeof(Gear));
					...
					// If the Parent is populated, remove the item from its Parent.
					if (objParent != null)
						objParent.Gears.Remove(objGear);
				}
				_objController.PopulateFocusList(treFoci);
				UpdateCharacterInfo();
				RefreshSelectedGear();
		...
		private void cmdDeleteVehicle_Click(object sender, EventArgs e)
		{
			...
			if (treVehicles.SelectedNode.Level == 1)
			{
				...
				Vehicle objVehicle = (Vehicle)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(Vehicle));
			...
			else if (treVehicles.SelectedNode.Level == 2)
			{
				...
				VehicleMod objMod = (VehicleMod)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(VehicleMod));

				if (objMod.Parent != null)
					objFoundVehicle = (Vehicle)objMod.Parent;
				...
				if (objMod != null)
				{
					...
					objFoundVehicle.VehicleMods.Remove(objMod);
					treVehicles.SelectedNode.Remove();
				}
				...
				if (!blnFound)
				{
					...
					foreach (Vehicle objCharacterVehicle in _objCharacter.Vehicles)
					{
						foreach (Gear objGear in objCharacterVehicle.Gears)
						{
							if (objGear.InternalId == treVehicles.SelectedNode.Tag.ToString())
							{
								...
								objCharacterVehicle.Gears.Remove(objGear);
								treVehicles.SelectedNode.Remove();
								blnFound = true;
								break;
							}
				...
				if (!blnFound)
				{
					...
					Vehicle objVehicle = (Vehicle)_objFunctions.FindEquipment(objVehicleNode.Tag.ToString(), _objCharacter.Vehicles, typeof(Vehicle));
			...
			else if (treVehicles.SelectedNode.Level == 3)
			{
				...
				if (!blnFound)
				{
					...
					Gear objGear = (Gear)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(Gear));
					if (objGear != null)
					{
						blnFound = true;
						((Gear)objGear.Parent).Gears.Remove(objGear);
						...
						foreach (VehicleMod objMod in objCharacterVehicle.VehicleMods)
						{
							foreach (Cyberware objCyberware in objMod.Cyberwares)
							{
								if (objCyberware.InternalId == treVehicles.SelectedNode.Tag.ToString())
								{
									...
									objMod.Cyberwares.Remove(objCyberware);
									treVehicles.SelectedNode.Remove();
									break;
								}
							}
			...
			else if (treVehicles.SelectedNode.Level == 4)
			{
							...
							if (!blnFound)
							{
								...
								if (objWeapon.Weapons.Count > 0)
								{
									foreach (Weapon objUnderbarrelWeapon in objWeapon.Weapons)
									{
										...
										{
											objWeapon.Weapons.Remove(objUnderbarrelWeapon);
											treVehicles.SelectedNode.Remove();
											break;
										}
									}
				...
				if (!blnFound)
				{
					...
					if (objGear.Parent != null)
						objFoundVehicle = (Vehicle)objGear.Parent;
		...
		{
			Gear objGear = (Gear)_objFunctions.FindEquipment(treGear.SelectedNode.Tag.ToString(), _objCharacter.Gear, typeof(Gear));
			Gear objParent = (Gear)_objFunctions.FindEquipment(treGear.SelectedNode.Parent.Tag.ToString(), _objCharacter.Gear, typeof(Gear));
		...
		private void cmdGearSplitQty_Click(object sender, EventArgs e)
		{
			...
			XmlNode objNode = objXmlDocument.SelectSingleNode("/chummer/gears/gear[id = \"" + objSelectedGear.ExternalId + "\"]");
		...
		private void cmdGearMergeQty_Click(object sender, EventArgs e)
		{
			...
			foreach (Gear objCharacterGear in _objCharacter.Gear)
			{
				...
				{
					blnMatch = true;
					if (objCharacterGear.Gears.Count == objGear.Gears.Count)
					{
						for (int i = 0; i <= objCharacterGear.Gears.Count - 1; i++)
						{
							if (objCharacterGear.Gears[i].Name != objGear.Gears[i].Name || objCharacterGear.Gears[i].Extra != objGear.Gears[i].Extra || objCharacterGear.Gears[i].Rating != objGear.Gears[i].Rating)
							{
								blnMatch = false;
								break;
							}
			...
			Gear objSelectedGear = (Gear)_objFunctions.FindEquipment(frmPickItem.SelectedItem, _objCharacter.Gear, typeof(Gear));
		...
		private void cmdGearMoveToVehicle_Click(object sender, EventArgs e)
		{
			...
			foreach (Gear objVehicleGear in objVehicle.Gears)
			{
				if (objVehicleGear.Name == objSelectedGear.Name && objVehicleGear.Category == objSelectedGear.Category && objVehicleGear.Rating == objSelectedGear.Rating && objVehicleGear.Extra == objSelectedGear.Extra && objVehicleGear.GivenName == objSelectedGear.GivenName && objVehicleGear.Notes == objSelectedGear.Notes)
				{
					blnMatch = true;
					objFoundGear = objVehicleGear;
					if (objVehicleGear.Gears.Count == objSelectedGear.Gears.Count)
					{
						for (int i = 0; i <= objVehicleGear.Gears.Count - 1; i++)
						{
							if (objVehicleGear.Gears[i].Name != objSelectedGear.Gears[i].Name || objVehicleGear.Gears[i].Extra != objSelectedGear.Gears[i].Extra || objVehicleGear.Gears[i].Rating != objSelectedGear.Gears[i].Rating)
							{
								blnMatch = false;
								break;
							}
			...
			if (objSelectedGear.Quantity == 0)
			{
				if (objSelectedGear.Parent != null)
					((Gear)objSelectedGear.Parent).Gears.Remove(objSelectedGear);
				else
		...
		private void cmdVehicleMoveToInventory_Click(object sender, EventArgs e)
		{
			...
			foreach (Vehicle objCharacterVehicle in _objCharacter.Vehicles)
			{
				...
				foreach (VehicleMod objVehicleMod in objCharacterVehicle.VehicleMods)
				{
			...
			else
			{
				...
				Gear objSelectedGear = (Gear)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(Gear));
				...
				foreach (Gear objCharacterGear in _objCharacter.Gear)
				{
					if (objCharacterGear.Name == objSelectedGear.Name && objCharacterGear.Category == objSelectedGear.Category && objCharacterGear.Rating == objSelectedGear.Rating && objCharacterGear.Extra == objSelectedGear.Extra && objCharacterGear.GivenName == objSelectedGear.GivenName && objCharacterGear.Notes == objSelectedGear.Notes)
					{
						blnMatch = true;
						objFoundGear = objCharacterGear;
						if (objCharacterGear.Gears.Count == objSelectedGear.Gears.Count)
						{
							for (int i = 0; i <= objCharacterGear.Gears.Count - 1; i++)
							{
								if (objCharacterGear.Gears[i].Name != objSelectedGear.Gears[i].Name || objCharacterGear.Gears[i].Extra != objSelectedGear.Gears[i].Extra || objCharacterGear.Gears[i].Rating != objSelectedGear.Gears[i].Rating)
								{
									blnMatch = false;
									break;
								}
				...
				if (objSelectedGear.Quantity == 0)
				{
					...
					treVehicles.SelectedNode.Remove();
					foreach (Vehicle objCharacterVehicle in _objCharacter.Vehicles)
						objCharacterVehicle.Gears.Remove(objSelectedGear);
				}
				else
		...
		private void cmdVehicleGearReduceQty_Click(object sender, EventArgs e)
		{
			...
			objGear = (Gear)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(Gear));
			objParent = (Gear)objGear.Parent;

			if (objGear.Parent != null)
				objVehicle = (Vehicle)objGear.Parent;
			...
			else
			{
				...
				if (objParent == null)
				{
					objVehicle.Gears.Remove(objGear);
					treVehicles.SelectedNode.Remove();
				}
				else
				{
		...
		private void cmdAddQuality_Click(object sender, EventArgs e)
		{
			...
			int intKarmaCost = objQuality.Cost * _objOptions.KarmaQuality;
		...
		private void cmdDeleteQuality_Click(object sender, EventArgs e)
		{
			...
			else
			{
				...
				int intKarmaCost = (objQuality.Cost * _objOptions.KarmaQuality) * -1;
				if (intKarmaCost > _objCharacter.Karma)
				{
					MessageBox.Show(LanguageManager.Instance.GetString("Message_NotEnoughKarma"), LanguageManager.Instance.GetString("MessageTitle_NotEnoughKarma"), MessageBoxButtons.OK, MessageBoxIcon.Information);
					return;
		...
		private void cmdSwapQuality_Click(object sender, EventArgs e)
		{
			...
			bool blnAddItem = true;
			int intKarmaCost = (objNewQuality.Cost - objQuality.Cost) * _objOptions.KarmaQuality;
		...
		private void cmdAddVehicleLocation_Click(object sender, EventArgs e)
		{
			...
			try
			{
				if (treVehicles.SelectedNode.Level == 1)
				{
					objVehicle = (Vehicle)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(Vehicle));
				}
				else
				{
		...
		private void tsAddArmorMod_Click(object sender, EventArgs e)
		{
			...
			XmlNode objXmlArmor = objXmlDocument.SelectSingleNode("/chummer/armors/armor[id = \"" + objArmor.ExternalId + "\"]");
			...
			if (objXmlArmor["addmodcategory"] != null)
		...
		private void tsVehicleAddMod_Click(object sender, EventArgs e)
		{
			...
			objSelectedVehicle.VehicleMods.Add(objMod);
			...
			{
				MessageBox.Show(LanguageManager.Instance.GetString("Message_CapacityReached"), LanguageManager.Instance.GetString("MessageTitle_CapacityReached"), MessageBoxButtons.OK, MessageBoxIcon.Information);
				objSelectedVehicle.VehicleMods.Remove(objMod);
				if (frmPickVehicleMod.AddAgain)
		...
		private void tsVehicleAddWeaponAccessory_Click(object sender, EventArgs e)
		{
			...
			Weapon objWeapon = (Weapon)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(Weapon));
			if (objWeapon != null)
				blnWeaponFound = true;
			...
			XmlNode objXmlWeapon = objXmlDocument.SelectSingleNode("/chummer/weapons/weapon[id = \"" + treVehicles.SelectedNode.Tag + "\"]");
		...
		private void tsVehicleAddUnderbarrelWeapon_Click(object sender, EventArgs e)
		{
			// Attempt to locate the selected VehicleWeapon.
			bool blnFound = false;
			Weapon objSelectedWeapon = (Weapon)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(Weapon));
		...
		private void tsVehicleSensorAddAsPlugin_Click(object sender, EventArgs e)
		{
			...
			bool blnFound = false;
			Gear objSensor = (Gear)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(Gear));
			if (objSensor != null)
				blnFound = true;
			...
			foreach (Vehicle objVehicle in _objCharacter.Vehicles)
			{
				foreach (VehicleMod objMod in objVehicle.VehicleMods)
				{
					foreach (Weapon objVehicleWeapon in objMod.Weapons)
					{
						...
						if (objVehicleWeapon.Weapons.Count > 0)
						{
							foreach (Weapon objUnderbarrelWeapon in objVehicleWeapon.Weapons)
							{
								if (objUnderbarrelWeapon.InternalId == treVehicles.SelectedNode.Tag.ToString())
								{
									objWeapon = objUnderbarrelWeapon;
			...
			try
			{
				if (treCyberware.SelectedNode.Level > 0)
				{
					...
					foreach (Cyberware objCharacterCyberware in _objCharacter.Cyberware)
					{
						...
						foreach (Cyberware objChildCyberware in objCharacterCyberware.Cyberwares)
						{
			...
			try
			{
				...
				else if (treArmor.SelectedNode.Level == 2)
				{
					...
					if (blnIsMod)
					{
						...
						int intOriginal = ((Armor)objMod.Parent).TotalCost;
						...
						// Create the Expense Log Entry for the sale.
						int intAmount = Convert.ToInt32(Convert.ToDouble(intOriginal - ((Armor)objMod.Parent).TotalCost, GlobalOptions.Instance.CultureInfo) * frmSell.SellPercent);
						ExpenseLogEntry objExpense = new ExpenseLogEntry();
				...
				else if (treArmor.SelectedNode.Level > 2)
				{
					...
					Gear objGear = new Gear(_objCharacter);
					Gear objParent = new Gear(_objCharacter);
					objGear = (Gear)_objFunctions.FindEquipment(treArmor.SelectedNode.Tag.ToString(), _objCharacter.Armor, typeof(Gear));
					objParent = (Gear)objGear.Parent;
			...
			try
			{
				if (treGear.SelectedNode.Level > 0)
				{
					...
					foreach (Gear objCharacterGear in _objCharacter.Gear)
					{
						...
						foreach (Gear objChildGear in objCharacterGear.Gears)
						{
						...
						foreach (VehicleMod objMod in objCharacterVehicle.VehicleMods)
						{
							if (objMod.InternalId == treVehicles.SelectedNode.Tag.ToString())
							{
								...
								else
								{
									...
									objCharacterVehicle.VehicleMods.Remove(objMod);
									treVehicles.SelectedNode.Remove();
		...
		private void tsWeaponName_Click(object sender, EventArgs e)
		{
			...
			objWeapon.GivenName = frmPickText.SelectedValue;
		...
		private void tsGearName_Click(object sender, EventArgs e)
		{
			...
			objGear.GivenName = frmPickText.SelectedValue;
			treGear.SelectedNode.Text = objGear.DisplayName;
		...
		private void tsUndoNuyenExpense_Click(object sender, EventArgs e)
		{
			...
			switch (objEntry.Undo.NuyenType)
			{
						...
						else
						{
							foreach (Cyberware objChild in objCyberware.Cyberwares)
							{
								if (objChild.InternalId == objEntry.Undo.ObjectId)
								{
									// Remove the Improvements created by the item.
									_objImprovementManager.RemoveImprovements(Improvement.ImprovementSource.Cyberware, objChild.InternalId);
									objCyberware.Cyberwares.Remove(objChild);
				...
				case NuyenExpenseType.AddGear:
					...
					Gear objGear = (Gear)objCommon.FindEquipment(objEntry.Undo.ObjectId, _objCharacter.Gear, typeof(Gear));
					...
					foreach (Vehicle objVehicle in _objCharacter.Vehicles)
					{
						foreach (VehicleMod objMod in objVehicle.VehicleMods)
						{
							if (objMod.InternalId == objEntry.Undo.ObjectId)
							{
								...
								objVehicle.VehicleMods.Remove(objMod);
					...
					foreach (Vehicle objVehicle in _objCharacter.Vehicles)
					{
						foreach (Gear objVehicleGear in objVehicle.Gears)
						{
							if (objVehicleGear.InternalId == objEntry.Undo.ObjectId)
							{
								...
								if (objVehicleGear.Quantity <= 0)
								{
									objVehicle.Gears.Remove(objVehicleGear);
								}
							...
							{
								// Look in child items.
								foreach (Gear objChild in objVehicleGear.Gears)
								{
									if (objChild.InternalId == objEntry.Undo.ObjectId)
									{
										// Deduct the Qty from the Gear.
										...
										if (objChild.Quantity <= 0)
										{
											objVehicleGear.Gears.Remove(objChild);
										}
									...
									else
									{
										foreach (Gear objSubChild in objChild.Gears)
										{
											if (objSubChild.InternalId == objEntry.Undo.ObjectId)
											{
												...
												if (objSubChild.Quantity <= 0)
												{
													objChild.Gears.Remove(objSubChild);
												}
				...
				case NuyenExpenseType.AddArmor:
					...
					Armor objArmor = (Armor)objCommon.FindEquipment(objEntry.Undo.ObjectId, _objCharacter.Armor, typeof(Armor));
				...
				case NuyenExpenseType.AddArmorGear:
					...
					foreach (Armor objFoundArmor in _objCharacter.Armor)
					{
						foreach (Gear objArmorGear in objFoundArmor.Gears)
						{
							if (objArmorGear.InternalId == objEntry.Undo.ObjectId)
							{
								...
								if (objArmorGear.Quantity <= 0)
								{
									...
									objFoundArmor.Gears.Remove(objArmorGear);
							...
							{
								// Look in child items.
								foreach (Gear objChild in objArmorGear.Gears)
								{
									if (objChild.InternalId == objEntry.Undo.ObjectId)
									{
										// Deduct the Qty from the Gear.
										...
										if (objChild.Quantity <= 0)
										{
											...
											objArmorGear.Gears.Remove(objChild);
										}
				...
				case NuyenExpenseType.AddCyberwareGear:
					...
					Gear objFoundGear = (Gear)_objFunctions.FindEquipment(objEntry.Undo.ObjectId, _objCharacter.Cyberware, typeof(Gear));
					_objFunctions.DeleteGear(objFoundGear, treWeapons, _objImprovementManager);

					if (objFoundGear.Parent != null)
						objFoundCyberware = (Cyberware)objFoundGear.Parent;

					if (objFoundGear.Parent == null)
						objFoundCyberware.Gears.Remove(objFoundGear);
					else
						((Gear)objFoundGear.Parent).Gears.Remove(objFoundGear);
				...
				case NuyenExpenseType.AddWeaponGear:
					...
					Gear objFoundAccGear = (Gear)_objFunctions.FindEquipment(objEntry.Undo.ObjectId, _objCharacter.Weapons, typeof(Gear));
					_objFunctions.DeleteGear(objFoundAccGear, treWeapons, _objImprovementManager);

					if (objFoundAccGear.Parent != null)
						objFoundAccessory = (WeaponAccessory)objFoundAccGear.Parent;

					if (objFoundAccGear.Parent == null)
						objFoundAccessory.Gears.Remove(objFoundAccGear);
					else
						((Gear)objFoundAccGear.Parent).Gears.Remove(objFoundAccGear);
		...
		private void tsArmorModNotes_Click(object sender, EventArgs e)
		{
			...
			{
				bool blnFound = false;
				ArmorMod objArmorMod = (ArmorMod)_objFunctions.FindEquipment(treArmor.SelectedNode.Tag.ToString(), _objCharacter.Armor, typeof(ArmorMod));
				if (objArmorMod != null)
					blnFound = true;
		...
		private void tsArmorGearNotes_Click(object sender, EventArgs e)
		{
			...
			{
				bool blnFound = false;
				Gear objArmorGear = (Gear)_objFunctions.FindEquipment(treArmor.SelectedNode.Tag.ToString(), _objCharacter.Armor, typeof(Gear));
		...
		private void tsCyberwareNotes_Click(object sender, EventArgs e)
		{
			...
			{
				bool blnFound = false;
				Cyberware objCyberware = (Cyberware)_objFunctions.FindEquipment(treCyberware.SelectedNode.Tag.ToString(), _objCharacter.Cyberware, typeof(Cyberware));
				if (objCyberware != null)
					blnFound = true;
		...
		private void tsVehicleName_Click(object sender, EventArgs e)
		{
			...
			objVehicle.GivenName = frmPickText.SelectedValue;
		...
		private void tsVehicleAddCyberware_Click(object sender, EventArgs e)
		{
			...
			if (objMod.Parent != null)
				objVehicle = (Vehicle)objMod.Parent;
			...
			treVehicles.SelectedNode.Nodes.Add(objNode);
			treVehicles.SelectedNode.Expand();
			objMod.Cyberwares.Add(objCyberware);
		...
		private void tsCyberwareAddGear_Click(object sender, EventArgs e)
		{
			...
			objCyberware.Gears.Add(objNewGear);
		...
		private void tsCyberwareGearMenuAddAsPlugin_Click(object sender, EventArgs e)
		{
			...
			// Locate the Vehicle Sensor Gear.
			bool blnFound = false;
			Gear objSensor = (Gear)_objFunctions.FindEquipment(treCyberware.SelectedNode.Tag.ToString(), _objCharacter.Cyberware, typeof(Gear));
			if (objSensor != null)
				blnFound = true;
		...
		{
			WeaponMod objMod = (WeaponMod)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(WeaponMod));
		...
		private void treQualities_AfterSelect(object sender, TreeViewEventArgs e)
		{
			...
			lblQualityBP.Text = (objQuality.Cost * _objOptions.KarmaQuality).ToString() + " " + LanguageManager.Instance.GetString("String_Karma");
		}
		#endregion
				...
				else if (treArmor.SelectedNode.Level > 1)
				{
					...
					if (objMod != null)
					{
						...
						if (chkArmorEquipped.Checked)
						{
							...
							if (objMod.Bonus != null && ((Armor)objMod.Parent).Equipped)
								_objImprovementManager.CreateImprovements(Improvement.ImprovementSource.ArmorMod, objMod.InternalId, objMod.Bonus, false, objMod.Rating, objMod.DisplayNameShort);
						}
						else
						{
					...
					if (objGear.Parent != null)
						objFoundArmor = (Armor)objGear.Parent;
		...
		private void cmdReloadWeapon_Click(object sender, EventArgs e)
		{
			...
			if (objWeapon.AmmoCategory != "Grenade Launchers" && objWeapon.AmmoCategory != "Missile Launchers" && objWeapon.AmmoCategory != "Mortar Launchers")
			{
				...
				foreach (Gear objAmmo in _objCharacter.Gear)
				{
					...
					foreach (Gear objChild in objAmmo.Gears)
					{
						if (objChild.Quantity > 0)
						{
			...
			else
			{
				if (objWeapon.AmmoCategory == "Grenade Launchers")
				{
					...
					foreach (Gear objAmmo in _objCharacter.Gear)
					{
						if (objAmmo.Quantity > 0)
						{
							if (objAmmo.Category == "Ammunition" && objAmmo.AmmoType == AmmoType.Minigrenade)
								lstAmmo.Add(objAmmo);
						}
						foreach (Gear objChild in objAmmo.Gears)
						{
							if (objChild.Quantity > 0)
							{
								if (objChild.Category == "Ammunition" && objChild.AmmoType == AmmoType.Minigrenade)
									lstAmmo.Add(objChild);
							}
						}
					}
				...
				if (objWeapon.AmmoCategory == "Missile Launchers")
				{
					...
					foreach (Gear objAmmo in _objCharacter.Gear)
					{
						if (objAmmo.Quantity > 0)
						{
							if (objAmmo.Category == "Ammunition" && (objAmmo.AmmoType == AmmoType.Missile || objAmmo.AmmoType == AmmoType.Rocket))
								lstAmmo.Add(objAmmo);
						}
						foreach (Gear objChild in objAmmo.Gears)
						{
							if (objChild.Quantity > 0)
							{
								if (objChild.Category == "Ammunition" && (objChild.AmmoType == AmmoType.Missile || objChild.AmmoType == AmmoType.Rocket))
									lstAmmo.Add(objChild);
							}
						}
					}
				...
				if (objWeapon.AmmoCategory == "Mortar Launchers")
				{
					...
					foreach (Gear objAmmo in _objCharacter.Gear)
					{
						if (objAmmo.Quantity > 0)
						{
							if (objAmmo.Category == "Ammunition" && objAmmo.AmmoType == AmmoType.Mortar)
								lstAmmo.Add(objAmmo);
						}
						foreach (Gear objChild in objAmmo.Gears)
						{
							if (objChild.Quantity > 0)
							{
								if (objChild.Category == "Ammunition" && objChild.AmmoType == AmmoType.Mortar)
									lstAmmo.Add(objChild);
							}
						}
					}
		...
		private void chkWeaponAccessoryInstalled_CheckedChanged(object sender, EventArgs e)
		{
			...
			else
			{
				...
				if (blnMod)
					objMod.Equipped = chkWeaponAccessoryInstalled.Checked;
				else
				{
					...
					if (!blnUnderbarrel)
					{
						...
						try
						{
							objSelectedGear = (Gear)_objFunctions.FindEquipment(treWeapons.SelectedNode.Tag.ToString(), _objCharacter.Weapons, typeof(Gear));
		...
		private void chkIncludedInWeapon_CheckedChanged(object sender, EventArgs e)
		{
			...
			if (blnAccessory)
			{
				objAccessory.IncludedInParent = chkIncludedInWeapon.Checked;
			}
			else
			{
				// Locate the selected Weapon Modification.
				WeaponMod objMod = (WeaponMod)_objFunctions.FindEquipment(treWeapons.SelectedNode.Tag.ToString(), _objCharacter.Weapons, typeof(WeaponMod));
				if (objMod != null)
					objMod.IncludedInParent = chkIncludedInWeapon.Checked;
		...
		private void chkGearEquipped_CheckedChanged(object sender, EventArgs e)
		{
			...
			try
			{
				objSelectedGear = (Gear)_objFunctions.FindEquipment(treGear.SelectedNode.Tag.ToString(), _objCharacter.Gear, typeof(Gear));
		...
		private void cboWeaponAmmo_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			Weapon objWeapon = new Weapon(_objCharacter);
			objWeapon = (Weapon)_objFunctions.FindEquipment(treWeapons.SelectedNode.Tag.ToString(), _objCharacter.Weapons, typeof(Weapon));
		...
		private void chkGearHomeNode_CheckedChanged(object sender, EventArgs e)
		{
			...
			objGear = (Commlink)_objFunctions.FindEquipment(treGear.SelectedNode.Tag.ToString(), _objCharacter.Gear, typeof(Commlink));
		...
		private void cmdWeaponMoveToVehicle_Click(object sender, EventArgs e)
		{
			...
			List<Equipment> lstVehicles = new List<Equipment>();
			foreach (Vehicle objCharacterVehicle in _objCharacter.Vehicles)
			{
				foreach (VehicleMod objVehicleMod in objCharacterVehicle.VehicleMods)
				{
					// Only add a Vehicle to the list if it has a Weapon Mount or Mechanical Arm.
					if (objVehicleMod.WeaponMount)
					{
			...
			VehicleMod objMod = new VehicleMod(_objCharacter);
			foreach (VehicleMod objCharacterMod in objVehicle.VehicleMods)
			{
		...
		private void chkIncludedInArmor_CheckedChanged(object sender, EventArgs e)
		{
			...
			ArmorMod objMod = (ArmorMod)_objFunctions.FindEquipment(treArmor.SelectedNode.Tag.ToString(), _objCharacter.Armor, typeof(ArmorMod));
			if (objMod != null)
				objMod.IncludedInParent = chkIncludedInArmor.Checked;
		...
		private void cmdReloadVehicleWeapon_Click(object sender, EventArgs e)
		{
			...
			if (objWeapon != null)
				objVehicle = (Vehicle)objWeapon.Parent;
			...
			{
				// This is a standard Weapon, so consume traditional Ammunition.
				foreach (Gear objAmmo in objVehicle.Gears)
				{
					if (objAmmo.Quantity > 0)
					{
						if (objAmmo.Category == "Ammunition" && objAmmo.Extra == objWeapon.AmmoCategory)
		...
		private void chkVehicleWeaponAccessoryInstalled_CheckedChanged(object sender, EventArgs e)
		{
			...
			else
			{
				...
				else
				{
					...
					VehicleMod objVehicleMod = (VehicleMod)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(VehicleMod));
					if (objVehicleMod != null)
						objVehicleMod.Equipped = chkVehicleWeaponAccessoryInstalled.Checked;
					else
					{
		...
		private void chkVehicleHomeNode_CheckedChanged(object sender, EventArgs e)
		{
			...
			{
				Commlink objGear = new Commlink(_objCharacter);
				objGear = (Commlink)_objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, typeof(Commlink));
		...
		private void treFoci_AfterCheck(object sender, TreeViewEventArgs e)
		{
			if (e.Node.Checked)
			{
				...
				objSelectedFocus = (Gear)_objFunctions.FindEquipment(e.Node.Tag.ToString(), _objCharacter.Gear, typeof(Gear));
				...
				else
				{
					...
					Gear objStackGear = (Gear)_objFunctions.FindEquipment(objStack.GearId, _objCharacter.Gear, typeof(Gear));
			...
			else
			{
				...
				Gear objGear = new Gear(_objCharacter);
				objGear = (Gear)_objFunctions.FindEquipment(objFocus.GearId, _objCharacter.Gear, typeof(Gear));
		...
		private void cboTradition_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			XmlNode objXmlTradition = objXmlDocument.SelectSingleNode("/chummer/traditions/tradition[id = \"" + cboTradition.SelectedValue + "\"]");
			lblDrainAttributes.Text = objXmlTradition["drain"].InnerText;
			...
			_objCharacter.MagicTradition = Guid.Parse(cboTradition.SelectedValue.ToString());

			foreach (SpiritControl objSpiritControl in panSpirits.Controls)
				objSpiritControl.RebuildSpiritList(Guid.Parse(cboTradition.SelectedValue.ToString()));
		...
		private void cboStream_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			XmlNode objXmlTradition = objXmlDocument.SelectSingleNode("/chummer/traditions/tradition[id = \"" + cboStream.SelectedValue + "\"]");
			lblFadingAttributes.Text = objXmlTradition["drain"].InnerText;
			...
			_objCharacter.TechnomancerStream = Guid.Parse(cboStream.SelectedValue.ToString());

			foreach (SpiritControl objSpriteControl in panSprites.Controls)
				objSpriteControl.RebuildSpiritList(Guid.Parse(cboStream.SelectedValue.ToString()));
		...
		private void cmdEdgeSpent_Click(object sender, EventArgs e)
		{
            ...
            _objImprovementManager.CreateImprovement("EDG", Improvement.ImprovementSource.EdgeUse, "edgeuse", Improvement.ImprovementType.Attribute, "", false, 0, 1, 0, 0, intEdgeUsed);
			_objImprovementManager.Commit();
			UpdateCharacterInfo();
		...
		public void MetatypeSelected()
		{
			...
			lblMAG.Text = _objCharacter.MAG.TotalValue.ToString();
			lblRES.Text = _objCharacter.RES.TotalValue.ToString();
		...
		public void UpdateCharacterInfo()
		{
			...
			if (!_blnSkipUpdate)
			{
				...
				if (_objCharacter.MetatypeCategory == "Cyberzombie")
				{
                    ...
                    _objImprovementManager.CreateImprovement("BOD", Improvement.ImprovementSource.Cyberzombie, "Cyberzombie Attributes", Improvement.ImprovementType.Attribute, "", false, 0, 1, 0, intESSModifier);
                    _objImprovementManager.CreateImprovement("AGI", Improvement.ImprovementSource.Cyberzombie, "Cyberzombie Attributes", Improvement.ImprovementType.Attribute, "", false, 0, 1, 0, intESSModifier);
                    _objImprovementManager.CreateImprovement("REA", Improvement.ImprovementSource.Cyberzombie, "Cyberzombie Attributes", Improvement.ImprovementType.Attribute, "", false, 0, 1, 0, intESSModifier);
                    _objImprovementManager.CreateImprovement("STR", Improvement.ImprovementSource.Cyberzombie, "Cyberzombie Attributes", Improvement.ImprovementType.Attribute, "", false, 0, 1, 0, intESSModifier);
                    _objImprovementManager.CreateImprovement("CHA", Improvement.ImprovementSource.Cyberzombie, "Cyberzombie Attributes", Improvement.ImprovementType.Attribute, "", false, 0, 1, 0, intESSModifier);
                    _objImprovementManager.CreateImprovement("INT", Improvement.ImprovementSource.Cyberzombie, "Cyberzombie Attributes", Improvement.ImprovementType.Attribute, "", false, 0, 1, 0, intESSModifier);
                    _objImprovementManager.CreateImprovement("LOG", Improvement.ImprovementSource.Cyberzombie, "Cyberzombie Attributes", Improvement.ImprovementType.Attribute, "", false, 0, 1, 0, intESSModifier);
                    _objImprovementManager.CreateImprovement("WIL", Improvement.ImprovementSource.Cyberzombie, "Cyberzombie Attributes", Improvement.ImprovementType.Attribute, "", false, 0, 1, 0, intESSModifier);
				}
				...
				if (_objCharacter.MAGEnabled)
					cmdImproveMAG.Enabled = !(_objCharacter.MAG.TotalValue >= _objCharacter.MAG.MetatypeMaximum + _objCharacter.InitiateGrade - _objCharacter.EssencePenalty);
				else
					cmdImproveMAG.Enabled = false;

				if (_objCharacter.RESEnabled)
					cmdImproveRES.Enabled = !(_objCharacter.RES.TotalValue >= _objCharacter.RES.MetatypeMaximum + _objCharacter.SubmersionGrade - _objCharacter.EssencePenalty);
				else
				...
				if (intCMPenalty < 0)
                    _objImprovementManager.CreateImprovement("", Improvement.ImprovementSource.ConditionMonitor, "", Improvement.ImprovementType.ConditionMonitor, "", false, intCMPenalty);
				...
				// Armor Ratings.
				lblArmor.Text = _objCharacter.TotalArmorValue.ToString();
				lblCMArmor.Text = lblArmor.Text;
				string strArmorToolTip = "";
				strArmorToolTip = LanguageManager.Instance.GetString("Tip_Armor") + " (" + _objCharacter.ArmorValue.ToString() + ")";
				if (_objCharacter.ArmorValue != _objCharacter.TotalArmorValue)
					strArmorToolTip += " + " + LanguageManager.Instance.GetString("Tip_Modifiers") + " (" + (_objCharacter.TotalArmorValue - _objCharacter.ArmorValue).ToString() + ")";
				...
				{
                    _objImprovementManager.CreateImprovement("AGI", Improvement.ImprovementSource.ArmorEncumbrance, "Armor Encumbrance", Improvement.ImprovementType.Attribute, "", false, 0, 1, 0, 0, _objCharacter.ArmorEncumbrance);
                    _objImprovementManager.CreateImprovement("REA", Improvement.ImprovementSource.ArmorEncumbrance, "Armor Encumbrance", Improvement.ImprovementType.Attribute, "", false, 0, 1, 0, 0, _objCharacter.ArmorEncumbrance);
				}
				...
				// Initiative Dice.
				lblIP.Text = _objCharacter.InitiativeDice;
				string strIPTip = "";
				strIPTip = "1";
				if (Convert.ToInt32(_objImprovementManager.ValueOf(Improvement.ImprovementType.InitiativeDice)) > 0)
					strIPTip += " + " + LanguageManager.Instance.GetString("Tip_Modifiers") + " (" + _objImprovementManager.ValueOf(Improvement.ImprovementType.InitiativeDice).ToString() + ")";
				...
				// Movement.
				lblMovement.Text = _objCharacter.FullMovement();

				// Special Attribute-Only Test.
				lblComposure.Text = _objCharacter.Composure.ToString();
				strTip = LanguageManager.Instance.GetString("String_AttributeWILShort") + " (" + _objCharacter.WIL.TotalValue.ToString() + ") + " + LanguageManager.Instance.GetString("String_AttributeCHAShort") + " (" + _objCharacter.CHA.TotalValue.ToString() + ")";
                if (_objImprovementManager.ValueOf(Improvement.ImprovementType.Composure) != 0)
                    strTip += " + " + LanguageManager.Instance.GetString("Tip_Modifiers") + " (" + _objImprovementManager.ValueOf(Improvement.ImprovementType.Composure).ToString() + ")";
				tipTooltip.SetToolTip(lblComposure, strTip);
				lblJudgeIntentions.Text = _objCharacter.JudgeIntentions.ToString();
                strTip = LanguageManager.Instance.GetString("String_AttributeINTShort") + " (" + _objCharacter.INT.TotalValue.ToString() + ") + " + LanguageManager.Instance.GetString("String_AttributeCHAShort") + " (" + _objCharacter.CHA.TotalValue.ToString() + ")";
				...
				strTip = LanguageManager.Instance.GetString("String_AttributeSTRShort") + " (" + _objCharacter.STR.TotalValue.ToString() + ") + " + LanguageManager.Instance.GetString("String_AttributeBODShort") + " (" + _objCharacter.BOD.TotalValue.ToString() + ")";
				...
				strTip = LanguageManager.Instance.GetString("String_AttributeLOGShort") + " (" + _objCharacter.LOG.TotalValue.ToString() + ") + " + LanguageManager.Instance.GetString("String_AttributeWILShort") + " (" + _objCharacter.WIL.TotalValue.ToString() + ")";
                ...
                lblSurprise.Text = _objCharacter.Surprise.ToString();
                strTip = LanguageManager.Instance.GetString("String_AttributeREAShort") + " (" + _objCharacter.REA.TotalValue.ToString() + ") + " + LanguageManager.Instance.GetString("String_AttributeINTShort") + " (" + _objCharacter.INT.TotalValue.ToString() + ")";
                if (_objImprovementManager.ValueOf(Improvement.ImprovementType.Surprise) != 0)
                    strTip += " + " + LanguageManager.Instance.GetString("Tip_Modifiers") + " (" + _objImprovementManager.ValueOf(Improvement.ImprovementType.Surprise).ToString() + ")";
                tipTooltip.SetToolTip(lblSurprise, strTip);

                // Limits.
                lblMentalLimit.Text = _objCharacter.MentalLimit().ToString();
                lblPhysicalLimit.Text = _objCharacter.PhysicalLimit().ToString();
                lblSocialLimit.Text = _objCharacter.SocialLimit().ToString();
                lblAstralLimit.Text = _objCharacter.AstralLimit().ToString();
		...
		{
			Equipment objFoundEquipment = _objFunctions.FindEquipment(treCyberware.SelectedNode.Tag.ToString(), _objCharacter.Cyberware, null);
			...
			if (objFoundEquipment.GetType() == typeof(Cyberware))
			{
				Cyberware objCyberware = (Cyberware)objFoundEquipment;
			...
			else if (objFoundEquipment.GetType() == typeof(Gear) || objFoundEquipment.GetType() == typeof(Commlink) || objFoundEquipment.GetType() == typeof(OperatingSystem))
			{
				Gear objGear = (Gear)objFoundEquipment;
		...
		public void RefreshSelectedWeapon()
		{
			...
			Equipment objFoundEquipment = _objFunctions.FindEquipment(treWeapons.SelectedNode.Tag.ToString(), _objCharacter.Weapons, null);

			if (objFoundEquipment.GetType() == typeof(Weapon))
			{
				Weapon objWeapon = (Weapon)objFoundEquipment;

				_blnSkipRefresh = true;
				lblWeaponName.Text = objWeapon.DisplayNameShort;
				lblWeaponCategory.Text = objWeapon.DisplayCategory;
                lblWeaponAcc.Text = objWeapon.TotalAccuracy.ToString();
				...
				chkWeaponAccessoryInstalled.Enabled = objWeapon.IsUnderbarrelWeapon;
				chkWeaponAccessoryInstalled.Checked = objWeapon.Equipped;
				chkIncludedInWeapon.Enabled = false;
				chkIncludedInWeapon.Checked = objWeapon.IncludedInParent;
				...
				if (objWeapon.WeaponType == "Ranged" || (objWeapon.WeaponType == "Melee" && objWeapon.Ammo != "0"))
				{
					...
					for (int i = 1; i <= objWeapon.AmmoSlots; i++)
					{
						...
						objGear = (Gear)_objFunctions.FindEquipment(objWeapon.AmmoLoaded, _objCharacter.Gear, typeof(Gear));
			...
			else if (objFoundEquipment.GetType() == typeof(WeaponAccessory))
			{
				WeaponAccessory objSelectedAccessory = (WeaponAccessory)objFoundEquipment;
				Weapon objSelectedWeapon = (Weapon)objSelectedAccessory.Parent;
                ...
                chkIncludedInWeapon.Enabled = true;
				chkIncludedInWeapon.Checked = objSelectedAccessory.IncludedInParent;
			...
			else if (objFoundEquipment.GetType() == typeof(WeaponMod))
			{
				WeaponMod objSelectedMod = (WeaponMod)objFoundEquipment;
				Weapon objSelectedWeapon = (Weapon)objSelectedMod.Parent;
				...
				chkWeaponAccessoryInstalled.Enabled = true;
				chkWeaponAccessoryInstalled.Checked = objSelectedMod.Equipped;
                chkIncludedInWeapon.Enabled = true;
				chkIncludedInWeapon.Checked = objSelectedMod.IncludedInParent;
		...
		{
			Equipment objFoundEquipment = _objFunctions.FindEquipment(treArmor.SelectedNode.Tag.ToString(), _objCharacter.Armor, null);
			...
			if (treArmor.SelectedNode.Level == 0)
			{
				...
				foreach (Armor objArmor in _objCharacter.Armor)
				{
					if (objArmor.Equipped && (objArmor.Location == treArmor.SelectedNode.Text || objArmor.Location == string.Empty && treArmor.SelectedNode == treArmor.Nodes[0]))
						lblArmorEquipped.Text += objArmor.DisplayName + " (" + objArmor.TotalArmorValue.ToString() + ")\n";
				}
			...
			if (objFoundEquipment.GetType() == typeof(Armor))
			{
				Armor objArmor = (Armor)objFoundEquipment;
				_blnSkipRefresh = true;

				lblArmorValue.Text = objArmor.TotalArmorValue.ToString();
			...
			else if (objFoundEquipment.GetType() == typeof(ArmorMod))
			{
				ArmorMod objSelectedMod = (ArmorMod)objFoundEquipment;
				Armor objSelectedArmor = (Armor)objSelectedMod.Parent;

				lblArmorValue.Text = objSelectedMod.ArmorValue.ToString();
				...
				chkIncludedInArmor.Checked = objSelectedMod.IncludedInParent;
				_blnSkipRefresh = false;
			}
			else if (objFoundEquipment.GetType() == typeof(Gear) || objFoundEquipment.GetType() == typeof(Commlink) || objFoundEquipment.GetType() == typeof(OperatingSystem))
			{
				Gear objSelectedGear = (Gear)objFoundEquipment;
		...
		public void RefreshSelectedGear()
		{
			...
			Equipment objFoundEquipment = _objFunctions.FindEquipment(treGear.SelectedNode.Tag.ToString(), _objCharacter.Gear, null);

			if (objFoundEquipment.GetType() == typeof(Gear) || objFoundEquipment.GetType() == typeof(Commlink) || objFoundEquipment.GetType() == typeof(OperatingSystem))
			{
				Gear objGear = (Gear)objFoundEquipment;
		...
		private bool PickGear(bool blnAmmoOnly = false, Gear objStackGear = null, string strForceItemValue = "")
		{
			...
			if (blnMatchFound)
			{
				...
				foreach (Gear objPlugin in objStackWith.Gears)
		...
		private bool PickArmorGear(bool blnShowArmorCapacityOnly = false)
		{
			...
			{
				objSelectedArmor.Gears.Add(objSelectedGear);
				if (objSelectedArmor.CapacityRemaining < 0)
				{
					objSelectedArmor.Gears.Remove(objSelectedGear);
					MessageBox.Show(LanguageManager.Instance.GetString("Message_CapacityReached"), LanguageManager.Instance.GetString("MessageTitle_CapacityReached"), MessageBoxButtons.OK, MessageBoxIcon.Information);
					return frmPickGear.AddAgain;
				}
				else
		...
		private void RefreshSelectedVehicle()
		{
			...
			Equipment objFoundEquipment = _objFunctions.FindEquipment(treVehicles.SelectedNode.Tag.ToString(), _objCharacter.Vehicles, null);
			...
			if (objFoundEquipment.GetType() == typeof(Vehicle))
			{
				Vehicle objVehicle = (Vehicle)objFoundEquipment;
			...
			else if (objFoundEquipment.GetType() == typeof(VehicleMod))
			{
				VehicleMod objMod = (VehicleMod)objFoundEquipment;
			...
			else if (objFoundEquipment.GetType() == typeof(Cyberware))
			{
				Cyberware objCyberware = (Cyberware)objFoundEquipment;
			...
			else if (objFoundEquipment.GetType() == typeof(Weapon))
			{
				Weapon objWeapon = (Weapon)objFoundEquipment;
				Vehicle objCurrentVehicle = null;

				// Locate the Vehicle that the Weapon is attached to.
				Equipment objFindItem = objWeapon;
				do
				{
					objFindItem = _objFunctions.FindEquipment(objFindItem.Parent.InternalId, _objCharacter.Vehicles, null);
				} while (objFindItem.GetType() != typeof(Vehicle));
				objCurrentVehicle = (Vehicle)objFindItem;
				...
				if (objWeapon.WeaponType == "Ranged" || (objWeapon.WeaponType == "Melee" && objWeapon.Ammo != "0"))
				{
					...
					for (int i = 1; i <= objWeapon.AmmoSlots; i++)
					{
						...
						ListItem objAmmo = new ListItem();
						objWeapon.ActiveAmmoSlot = i;
						objVehicleGear = (Gear)_objFunctions.FindEquipment(objWeapon.AmmoLoaded, objCurrentVehicle.Gears, typeof(Gear));
						objAmmo.Value = i.ToString();

						string strPlugins = "";
						foreach (Vehicle objVehicle in _objCharacter.Vehicles)
						{
							foreach (Gear objCurrentAmmo in objVehicle.Gears)
							{
								if (objCurrentAmmo.InternalId == objWeapon.AmmoLoaded)
								{
									foreach (Gear objChild in objCurrentAmmo.Gears)
									{
										strPlugins += objChild.DisplayNameShort + ", ";
									}
								}
				...
				foreach (Gear objAutosoft in objCurrentVehicle.Gears)
				{
			...
			{
				WeaponAccessory objAccessory = (WeaponAccessory)objFoundEquipment;
				Weapon objWeapon = (Weapon)objAccessory.Parent;
				...
				chkVehicleIncludedInWeapon.Checked = objAccessory.IncludedInParent;
			...
			{
				WeaponMod objMod = (WeaponMod)objFoundEquipment;
				Weapon objWeapon = (Weapon)objMod.Parent;
				...
				chkVehicleIncludedInWeapon.Checked = objMod.IncludedInParent;
		...
		private void SetTooltips()
		{
			...
			tipTooltip.SetToolTip(cmdArmorIncrease, LanguageManager.Instance.GetString("Tip_ArmorDegradationPlus"));
			tipTooltip.SetToolTip(cmdArmorDecrease, LanguageManager.Instance.GetString("Tip_ArmorDegradationMinus"));
			// Weapon Tab.
            ...
            tipTooltip.SetToolTip(lblSurpriseLabel, LanguageManager.Instance.GetString("Tip_OtherSurprise"));
		...
		private void MoveControls()
		{
            ...
            lblWeaponAccLabel.Left = lblWeaponRC.Left + 95;
            lblWeaponAcc.Left = lblWeaponAPLabel.Left + intWidth + 6;
            ...
            intWidth = Math.Max(intWidth, lblSurpriseLabel.Width);
            ...
            intWidth = Math.Max(intWidth, lblMentalLimitLabel.Width);
            intWidth = Math.Max(intWidth, lblPhysicalLimitLabel.Width);
            intWidth = Math.Max(intWidth, lblSocialLimitLabel.Width);
            intWidth = Math.Max(intWidth, lblAstralLimitLabel.Width);
            ...
            lblSurprise.Left = lblCMPhysical.Left;
            ...
            lblMentalLimit.Left = lblCMPhysical.Left;
            lblSocialLimit.Left = lblCMPhysical.Left;
            lblPhysicalLimit.Left = lblCMPhysical.Left;
            lblAstralLimit.Left = lblCMPhysical.Left;
		...
		private void ChangeVehicleSensor(Vehicle objVehicle, bool blnIncrease)
		{
			...
			List<Weapon> lstWeapons = new List<Weapon>();
			List<TreeNode> lstWeaponNodes = new List<TreeNode>();
			foreach (Gear objCurrentGear in objVehicle.Gears)
			{
		...
		private TreeNode CreateSuiteCyberware(XmlNode objXmlItem, XmlNode objXmlNode, Grade objGrade, int intRating, bool blnAddToCharacter, Improvement.ImprovementSource objSource, string strType, Cyberware objParent = null)
		{
			...
			if (blnAddToCharacter)
				...
				objParent.Cyberwares.Add(objCyberware);
