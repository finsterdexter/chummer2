...
namespace Chummer
{
	...
	public class Attribute
	{
		...
		public int Value
		{
			...
			{
				_intValue = value;
                // Make sure the value never goes outside of the Metatype's limits.
                if (_intValue > MetatypeMaximum)
                    _intValue = MetatypeMaximum;
                if (_intValue < MetatypeMinimum)
                    _intValue = MetatypeMinimum;
			}
		}
		...
		public bool HasModifiers
		{
			get
			{
                ...
                if ((_strAbbrev == "MAG" || _strAbbrev == "RES") && _objCharacter.EssencePenalty > 0)
                    return true;
		...
		public int TotalValue
		{
			get
			{
                ...
                if (_strAbbrev == "MAG" || _strAbbrev == "RES")
		...
		public string ToolTip()
		{
            ...
            // If this is MAG or RES and the character has an Essence Penalty, add it to the tooltip.
            if ((_strAbbrev == "MAG" || _strAbbrev == "RES") && _objCharacter.EssencePenalty > 0)
                strModifier += " + " + LanguageManager.Instance.GetString("String_EssenceLoss") + " (-" + _objCharacter.EssencePenalty.ToString() + ")";
	...
	public class Quality
	{
        ...
        private XmlNode _nodConditional;
		...
		{
            _guidExternalID = Guid.Parse(objXmlQuality["id"].InnerText);
			_strName = objXmlQuality["name"].InnerText;
			_intCost = Convert.ToInt32(objXmlQuality["cost"].InnerText);
            ...
            _nodConditional = objXmlQuality["conditional"];
			...
			if (objXmlQuality["addweapon"] != null)
			{
            ...
            if (!objImprovementManager.CreateImprovements(Improvement.ImprovementSource.Quality, _guiID.ToString(), objXmlQuality["conditional"], false, 1, DisplayNameShort, true))
            {
                _guiID = Guid.Empty;
                return;
            }
		...
		public void Save(XmlTextWriter objWriter)
		{
            ...
            if (_nodConditional != null)
                objWriter.WriteRaw("<conditional>" + _nodConditional.InnerXml + "</conditional>");
            else
		...
		/// <summary>
		/// Amount of Karma the Quality costs.
		/// </summary>
		public int Cost
		{
			get
	...
	public class Skill
	{
		...
		private string _strName = "";
		private int _intRating = 0;
		private int _intRatingMaximum = 12;
		...
		private int CalculatedRating
		{
			get
			{
				...
				if (objImprovementManager.ValueOf(Improvement.ImprovementType.Skillwire) > 0 || (_objCharacter.KnowsoftAccess && _blnKnowledgeSkill))
				{
	...
	public class Spell
	{
		...
		public void Create(XmlNode objXmlSpellNode, Character objCharacter, TreeNode objNode, string strForcedValue = "", bool blnLimited = false, bool blnExtended = false, bool blnCreateImprovements = true)
		{
            _guidExternalID = Guid.Parse(objXmlSpellNode["id"].InnerText);
			...
			if (GlobalOptions.Instance.Language != "en-us")
			{
				...
				XmlNode objSpellNode = objXmlDocument.SelectSingleNode("/chummer/spells/spell[id = \"" + ExternalId + "\"]");
			...
			if (objXmlSpellNode["bonus"] != null && blnCreateImprovements)
			{
	...
	public class Power
	{
        ...
        public void Create(XmlNode objXmlPowerNode, Character objCharacter, int intRating = 1, string strForcedValue = "", bool blnCreateImprovements = true)
        {
            _guidExternalID = Guid.Parse(objXmlPowerNode["id"].InnerText);
            ...
            _decPointsPerLevel = Convert.ToDecimal(objXmlPowerNode["points"].InnerText, GlobalOptions.Instance.CultureInfo);
            _intRating = intRating;
            _nodBonus = objXmlPowerNode["bonus"];
            _nodConditional = objXmlPowerNode["conditional"];
            if (objXmlPowerNode["levels"] != null)
                _blnLevelsEnabled = Convert.ToBoolean(objXmlPowerNode["levels"].InnerText);
            if (objXmlPowerNode["doublecost"] != null)
                _blnDoubleCost = Convert.ToBoolean(objXmlPowerNode["doublecost"].InnerText);
            if (objXmlPowerNode["limit"] != null)
                _intMaxLevel = Convert.ToInt32(objXmlPowerNode["limit"].InnerText);

            if (GlobalOptions.Instance.Language != "en-us")
            {
                XmlDocument objXmlDocument = XmlManager.Instance.Load("powers.xml");
                XmlNode objPowerNode = objXmlDocument.SelectSingleNode("/chummer/powers/power[id = \"" + ExternalId + "\"]");
                if (objPowerNode != null)
                {
                    if (objPowerNode["translate"] != null)
                        _strAltName = objPowerNode["translate"].InnerText;
                    if (objPowerNode["altpage"] != null)
                        _strAltPage = objPowerNode["altpage"].InnerText;
                }
            }
            ...
            ImprovementManager objImprovementManager = new ImprovementManager(objCharacter);
            objImprovementManager.ForcedValue = strForcedValue;
            if (!objImprovementManager.CreateImprovements(Improvement.ImprovementSource.Power, _guiID.ToString(), objXmlPowerNode["bonus"], false, 1, DisplayNameShort))
            {
                _guiID = Guid.Empty;
                return;
            }
            ...
            // Conditional Improvements.
            if (!objImprovementManager.CreateImprovements(Improvement.ImprovementSource.Power, _guiID.ToString(), objXmlPowerNode["conditional"], false, 1, DisplayNameShort, true))
            {
                _guiID = Guid.Empty;
                return;
            }
	...
	public class TechProgram
	{
		...
		/// <param name="strForcedValue">Value to forcefully select for any ImprovementManager prompts.</param>
        /// <param name="blnCreateImprovements">Whether or not Improvements should be created.</param>
		public void Create(XmlNode objXmlProgramNode, Character objCharacter, TreeNode objNode, string strForcedValue = "", bool blnCreateImprovements = true)
		{
            _guidExternalID = Guid.Parse(objXmlProgramNode["id"].InnerText);
			...
			if (objXmlProgramNode["tags"] != null)
			{
			...
			if (objXmlProgramNode["bonus"] != null && blnCreateImprovements)
			{
	...
	public class TechProgramOption
	{
		...
		/// <param name="strForcedValue">Value to forcefully select for any ImprovementManager prompts.</param>
        /// <param name="blnCreateImprovements">Whether or not Improvements should be created.</param>
		public void Create(XmlNode objXmlOptionNode, Character objCharacter, TreeNode objNode, string strForcedValue = "", bool blnCreateImprovements = true)
		{
            _guidExternalID = Guid.Parse(objXmlOptionNode["id"].InnerText);
			...
			if (objXmlOptionNode["bonus"] != null && blnCreateImprovements)
			{
            ...
            {
                XmlDocument objXmlDocument = XmlManager.Instance.Load("programs.xml");
                XmlNode objOptionNode = objXmlDocument.SelectSingleNode("/chummer/options/option[id = \"" + ExternalId + "\"]");
                if (objOptionNode != null)
                {
                    if (objOptionNode["translate"] != null)
                        _strAltName = objOptionNode["translate"].InnerText;
                    if (objOptionNode["altpage"] != null)
                        _strAltPage = objOptionNode["altpage"].InnerText;
                }
            }
		}
	...
	public class MartialArt
	{
		...
		public void Load(XmlNode objNode)
		{
			...
			if (objNode["martialartadvantages"] != null)
			{
	...
	public class CritterPower
	{
		...
		/// <param name="strForcedValue">Value to forcefully select for any ImprovementManager prompts.</param>
        /// <param name="blnCreateImprovements">Whether or not Improvements should be created.</param>
		public void Create(XmlNode objXmlPowerNode, Character objCharacter, TreeNode objNode, int intRating = 0, string strForcedValue = "", bool blnCreateImprovements = true)
		{
            _guidExternalID = Guid.Parse(objXmlPowerNode["id"].InnerText);
			_strName = objXmlPowerNode["name"].InnerText;
			...
			if (blnCreateImprovements)
			{
                ...
                // Conditional Improvements.
                if (!objImprovementManager.CreateImprovements(Improvement.ImprovementSource.CritterPower, _guiID.ToString(), _nodConditional, false, 1, DisplayNameShort, true))
                {
                    _guiID = Guid.Empty;
                    return;
                }
		...
		public void Save(XmlTextWriter objWriter)
		{
			...
			if (_nodBonus != null)
				...
				objWriter.WriteElementString("bonus", "");
            if (_nodConditional != null)
                objWriter.WriteRaw("<conditional>" + _nodConditional + "</conditional>");
            else
                objWriter.WriteElementString("conditional", "");
			objWriter.WriteElementString("notes", _strNotes);
			objWriter.WriteEndElement();
